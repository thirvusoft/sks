...
function update_fields(frm,cdt,cdn) {
	let k=0;
	frappe.call({
		method: "sks.sks.doctype.ts_driver_delivery_trip.ts_driver_delivery_trip.get_buttons_data",
		args:{
    ...
    'details': function(frm,cdt,cdn){
        ...
        if(p.sales_invoice)
		{
			...
			if(data.mode_of_payment && data.amount && data.sales_invoice && data.delivery_status == "Delivered"){
				frappe.call({
					method:"sks.sks.doctype.ts_driver_delivery_trip.ts_driver_delivery_trip.payment_entry",
					args:{
			...
			frappe.call({
				method: "sks.sks.doctype.ts_driver_delivery_trip.ts_driver_delivery_trip.update_values",
				args:{
