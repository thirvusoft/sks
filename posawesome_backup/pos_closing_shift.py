...
def make_closing_shift_from_opening(opening_shift):
    ...
    pos = frappe.get_doc('POS Awesome Outstanding Amount', opening_shift.get("name"))
    modes = {}
    for i in pos.outstanding_amount:
        if(i.mode_of_payment not in list(modes.keys())):
            modes[i.mode_of_payment] = i.amount
        else:modes[i.mode_of_payment]+=i.amount
    pos_transactions = []
    taxes = []
    payments = []
    for detail in opening_shift.get("balance_details"):
        amount_add=0
        try:
            amount_add = modes[detail.get("mode_of_payment")]
        except:pass
        payments.append(frappe._dict({
            'mode_of_payment': detail.get("mode_of_payment"),
            'opening_amount': detail.get("amount") or 0,
            'expected_amount': detail.get("amount")+amount_add or 0
        }))
...
def submit_closing_shift(closing_shift):
    ...
    if(closing_shift_doc.docstatus == 1):
        pos=frappe.get_doc('POS Awesome Outstanding Amount',closing_shift_doc.pos_opening_shift)
